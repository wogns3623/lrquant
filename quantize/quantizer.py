...
class NonUniformQuantizer(nn.Module):
    def __init__(
        self,
        n_bits: int = 8,
        symmetric: bool = False,
        ...
        self.round_zero_point: torch.Tensor | None = None
    ...
    def fake_quant(self, x, scale: torch.Tensor, round_zero_point: torch.Tensor | None):
        if self.deficiency > 0:
            pad_zeros = torch.zeros((x.shape[0],self.deficiency),dtype=x.dtype,device=x.device)
            x = torch.cat((x,pad_zeros),dim=1)
        ...
        if self.group_size:
            ...
            dim1, dim2 = x.shape
            x = x.reshape(-1, self.group_size)
        x_int = round_ste(x.log() / scale) # round(w/h)
        if round_zero_point is not None:
            x_int = x_int.add(round_zero_point) # + z
        x_int = x_int.clamp(self.qmin, self.qmax) # clamp with qmin and qmax
    ...
    def per_token_dynamic_calibration(self, x: torch.Tensor):
        if self.group_size:
            if self.deficiency == 0:
                ...
                x = x.reshape(-1,self.group_size)
        reduce_shape = [-1]
        xmin: torch.Tensor = x.amin(reduce_shape, keepdim=True)
        xmax: torch.Tensor =  x.amax(reduce_shape, keepdim=True)
        if self.lwc:
            xmax = self.sigmoid(self.upbound_factor)*xmax # gamma * max(W)
            xmin = self.sigmoid(self.lowbound_factor)*xmin # beta * min(W)
        if self.symmetric:
            abs_max = torch.max(xmax.abs(),xmin.abs()) 
            scale: torch.Tensor = abs_max / (2**(self.n_bits-1)-1)
            self.scale = scale.clamp(min=CLIPMIN, max=1e4)
            zero_point: torch.Tensor = (2**(self.n_bits-1)-1)*torch.ones_like(self.scale)
        else:
            range = xmax - xmin # gamma * max(W) - beta * min(W)
            scale = range / (2**self.n_bits-1) # (gamma * max(W) - beta * min(W)) / (2^bit-1)
            self.scale = scale.clamp(min=CLIPMIN, max=1e4) # 최대 최소값 제한 1e-5 ~ 1e4
