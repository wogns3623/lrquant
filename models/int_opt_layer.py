...
from quantize.rlq_norm import RLQLayerNorm
...
class QuantOPTDecoderLayer(nn.Module):
    def __init__(
        ...
        self.self_attn_layer_norm = RLQLayerNorm(
    ...
    def smooth_and_quant_inplace(self):
        ...
        if self.let:
            ...
            smooth_ln_fcs_inplace(self.self_attn_layer_norm,[self.self_attn.q_proj, self.self_attn.k_proj, self.self_attn.v_proj],
                                    self.qkv_smooth_scale,self.qkv_smooth_shift,self.qkv_smooth_weight,self.qkv_smooth_act)
            smooth_ln_fcs_inplace(self.final_layer_norm,[self.fc1],
                                    self.fc1_smooth_scale,self.fc1_smooth_shift,self.fc1_smooth_weight,self.fc1_smooth_act)
            smooth_fc_fc_inplace(self.self_attn.v_proj,self.self_attn.out_proj,
                                self.out_smooth_scale,self.out_smooth_weight, self.out_smooth_act, self.out_smooth_shift)
